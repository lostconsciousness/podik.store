...
import requests
from main.models import Podik, NovaPost, Filters, Areas_and_costs, Offers, Users
...
def load_more(request):
    ...
    offers = {
        ...
        'pod':list(sorted(podfilter.qs.values(), key= lambda x: not x['available'])[total_item:total_item+50]),
        'pods':list(sorted(allObjects.filter(categoryId=220).values(), key= lambda x: not x['available'])[total_item:total_item+50]),
        'devices':list(sorted(allObjects.filter(categoryId=221).values(), key= lambda x: not x['available'])[total_item:total_item+50]),
        'cartridges':list(sorted(allObjects.filter(categoryId=222).values(), key= lambda x: not x['available'])[total_item:total_item+50]),
        'disposable':list(sorted(allObjects.filter(categoryId=239).values(), key= lambda x: not x['available'])[total_item:total_item+50]),
        'elf_bar':list(sorted(allObjects.filter(categoryId=287).values(), key= lambda x: not x['available'])[total_item:total_item+50]),
        'hqd':list(sorted(allObjects.filter(categoryId=288).values(), key= lambda x: not x['available'])[total_item:total_item+50]),
        'liquids':list(sorted(allObjects.filter(categoryId=208).values(), key= lambda x: not x['available'])[total_item:total_item+50]),
        'ukrainian_salt':list(sorted(allObjects.filter(categoryId=236).values(), key= lambda x: not x['available'])[total_item:total_item+50]),
        'premium_salt':list(sorted(allObjects.filter(categoryId=237).values(), key= lambda x: not x['available'])[total_item:total_item+50]),
    }
...
def homepage(request, tg_id):
    ...
    pods = serializers.serialize('json', sorted(podfilter.qs, key= lambda x: not x.available)[:50])
    pod_system = serializers.serialize('json', sorted(allObjects.filter(categoryId = 220), key= lambda x: not x.available)[:50])
    devices = serializers.serialize('json', sorted(allObjects.filter(categoryId = 221), key= lambda x: not x.available)[:50])
    disposable = serializers.serialize('json', sorted(allObjects.filter(categoryId = 239), key= lambda x: not x.available)[:50])
    cartridges = serializers.serialize('json', sorted(allObjects.filter(categoryId = 222), key= lambda x: not x.available)[:50])
    liquids = serializers.serialize('json', sorted(allObjects.filter(categoryId = 208), key= lambda x: not x.available)[:50])
    elf_bar = serializers.serialize('json', sorted(allObjects.filter(categoryId = 287), key= lambda x: not x.available)[:50])
    hqd = serializers.serialize('json', sorted(allObjects.filter(categoryId = 288), key= lambda x: not x.available)[:50])
    ukrainian_salt = serializers.serialize('json', sorted(allObjects.filter(categoryId = 236), key= lambda x: not x.available)[:50])
    premium_salt = serializers.serialize('json', sorted(allObjects.filter(categoryId = 237), key= lambda x: not x.available)[:50])
